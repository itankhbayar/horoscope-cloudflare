...
export type CompatibilityLang = 'en' | 'mn';
...
const SIGN_NAMES: Record<CompatibilityLang, Record<ZodiacSign, string>> = {
  en: {
...
const ELEMENT_NAMES: Record<CompatibilityLang, Record<string, string>> = {
  en: { fire: 'fire', earth: 'earth', air: 'air', water: 'water' },
  mn: { fire: 'гал', earth: 'шороо', air: 'агаар', water: 'ус' },
};

const MODALITY_NAMES: Record<CompatibilityLang, Record<string, string>> = {
...
const COPY: Record<CompatibilityLang, CopyBundle> = {
  en: {
...
function buildSummary(
  ...
  const aEl = ELEMENT_NAMES[lang][a.element] ?? a.element;
  const bEl = ELEMENT_NAMES[lang][b.element] ?? b.element;
  const aMod = MODALITY_NAMES[lang][a.modality] ?? a.modality;
  const bMod = MODALITY_NAMES[lang][b.modality] ?? b.modality;
  ...
  const summary = `${copy.pairing(aName, aEl, aMod, bName, bEl, bMod, tier)} ${lead}`.trim();
  return { summary, highlights };
}

/** Mirrors backend compatibilityService.computeSignCompatibility for guest landing previews. */
export function computeSignCompatibility(
  ...
  const elementScore = ELEMENT_AFFINITY[a.element]?.[b.element] ?? 50;
  const modalityScore = MODALITY_AFFINITY[a.modality]?.[b.modality] ?? 50;
